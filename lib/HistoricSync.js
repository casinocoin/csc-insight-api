...
var bitcore        = require('csc-bitcore-v0.1');
...
//  var Deserialize = require('csc-bitcore-v0.1/Deserialize');
var BAD_GEN_ERROR = 'Bad genesis block. Network mismatch between Insight and casinocoind? Insight is configured for:';
...
function HistoricSync(opts) {
  ...
  this.network = config.network === 'testnet' ? networks.testnet: networks.cscnet;
      ...
      async.doWhilst(
        ...
        function(err) {
          ...
          if ( self.blockChainHeight  === blockInfo.height ||
            ...
            oldtip = tip;
            if (!tip)
              throw new Error('Previous blockchain tip was not found on casinocoind. Please reset Insight DB. Tip was:'+tip)
...
HistoricSync.prototype.showSyncStartMessage = function() {
  ...
  info('Got ' + self.height +
    ' blocks in current DB, out of ' + self.blockChainHeight + ' block at casinocoind');

  if (self.blockExtractor) {
    info('casinocoind dataDir configured...importing blocks from .dat files');
